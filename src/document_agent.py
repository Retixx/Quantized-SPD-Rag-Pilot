...
An agent is only ever handed `store.get(document_id)`. It has no handle on any
other document, so isolation cannot be lost by forgetting a filter.
"""
...
from retrieval import DocumentIndexStore, Hit, format_chunks_for_prompt  # noqa: E402
...
def run_fixed_evidence_agent(client, question_id: str, document_id: str,
                             ...
                             hits: List[Hit], max_tokens: int = 512) -> Dict[str, Any]:
    ...
    cid = make_call_id(client.stage, client.precision, client.condition,
                       question_id, "document_agent", document_id)
    parsed, rec = client.call(
        call_id=cid, role="document_agent", question_id=question_id,
...
def run_end_to_end_agent(client, store: DocumentIndexStore, question_id: str,
                         document_id: str, question: str,
                         shared_tasks: List[Dict[str, Any]], top_k: int = 3,
                         max_search_rounds: int = 2, max_tokens: int = 512
                         ) -> Dict[str, Any]:
    # Fail fast if this agent was not assigned this document. store.get() is the
    # only handle it ever receives, so isolation cannot be lost downstream.
    ...
    for turn in range(max_search_rounds + 1):
        ...
        if action == "search" and budget_left and (parsed or {}).get("query"):
            q = str(parsed["query"]).strip()[:512]
            queries.append(q)
            new_hits = store.search(document_id, q, top_k=top_k)
            for h in new_hits:
                if h.chunk_id not in seen_ids:
                    seen_ids.append(h.chunk_id)
                    hits_all.append(h)
            continue
        if action == "finalize" or not budget_left or not parsed:
            final = parsed
            break

    if final is None or not (final.get("supported_facts") or final.get("insufficient_evidence")):
        # budget exhausted without a clean finalize: force one final extraction
        if not queries:
            queries.append(question[:512])
            for h in store.search(document_id, question, top_k=top_k):
                if h.chunk_id not in seen_ids:
                    seen_ids.append(h.chunk_id)
                    hits_all.append(h)
        ...
        forced.update(search_queries=queries, retrieved_chunk_ids=seen_ids,
                      ...
                      retrieval_success=bool(hits_all),
                      is_fixture=is_fixture or bool(forced.get("is_fixture")))
        return _drop_uncited(forced)
...
def _drop_uncited(out: Dict[str, Any]) -> Dict[str, Any]:
