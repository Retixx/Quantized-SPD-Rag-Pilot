...
def load_yaml(path: str | os.PathLike) -> Dict[str, Any]:
    """Load a config file. PyYAML when available, the faithful fallback otherwise.

    The original fallback silently mis-parsed the repo's own configs -- inline
    flow lists stayed strings, trailing comments leaked into values, and the
    nested `sampling.max_tokens` mapping was unreachable. A config that looks
    loaded but is wrong mis-configures a whole run with no error, which is worse
    than not loading at all.

    `_mini_yaml` now mirrors PyYAML's YAML 1.1 implicit resolvers, so plain
    scalars get the same types, and raises `MiniYamlUnsupported` on anything it
    cannot reproduce faithfully rather than guessing. `tests/test_config_yaml_
    fallback.py` asserts the two agree on every file in `configs/`.
    """
